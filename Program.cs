...
namespace DelegatesPracticeFilter
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            int[] evenArray = GetFiltered(array, delegate (int x) { return x % 2 == 0; });
            int[] notEvenArray = GetFiltered(array, delegate (int x) { return !(x % 2 == 0); });
            int[] has3Array = GetFiltered(array, delegate (int x) { return x.ToString().Contains("3"); });
            int[] hasSameNumberArray = GetFiltered(array, delegate (int x)
            {
                if (x < 10)
                    return true;
                else
